<template>
    ...
    <div class="flex-1 ml-64 flex flex-col">
      <main class="p-10">
        ...
        <section class="bg-[#121215] border border-white/5 p-8 rounded-xl shadow-2xl">
          ...
          <div v-if="users && users.length > 0" class="w-full overflow-hidden">
        ...
        <section class="bg-[#121215] border border-white/5 p-8 rounded-lg mt-6">
          <div class="flex flex-row items-center justify-between">
              ...
              <div class="flex flex-row items-center justify-between gap-3">
                <button @click="oppenToggle('createClassModal')" class="bg-transparent border border-[#00babc] text-[#00babc] font-bold py-2 px-4 rounded hover:bg-[#00babc] hover:text-[#121215] transition-all text-[11px] uppercase tracking-widest">
                  Create Class
                </button>
                <button @click="oppenToggle('AssignClassFormatorModal')" class="bg-[#00babc] text-[#121215] font-bold py-2 px-4 rounded hover:bg-[#00d1d3] transition-all text-[11px] uppercase tracking-widest">
                  Assign a Formator
                </button>
              </div>
          </div>
          <div v-if="classes && classes.length > 0" class="w-full overflow-hidden mt-8">
            <table class="w-full border-collapse">
              <thead>
                <tr class="text-left text-[10px] text-gray-500 uppercase tracking-[0.2em] border-b border-white/5">
              ...
              <tbody class="divide-y divide-white/[0.03]">
                <tr v-for="classe in classes" :key="classe.id" class="group hover:bg-white/[0.02] transition-colors">
                  ...
                  <td class="py-5 text-sm text-gray-400 font-mono">
                    {{ classe.formateur ? classe.formateur.fullname : 'Unassigned' }}
                  </td>
                  <td class="py-5 text-sm text-gray-400 font-mono">
                     {{ classe.promo }}
                  </td>
            ...
            <div v-else class="mt-10 pt-6 border-t border-white/5 flex items-center justify-between">
              <p class="text-xs text-gray-500 italic font-mono">// System_Ready_Waiting_For_Data</p>
            </div>
        </section>
        ...
        <section class="bg-[#121215] border border-white/5 p-8 rounded-lg mt-6">
          <div class="flex flex-row items-center justify-between">
            ...
            <button @click="oppenToggle('detectAbsencesModal')" class="bg-[#00babc] text-[#121215] font-bold py-2 px-4 rounded hover:bg-[#00d1d3] transition-all text-[11px] uppercase tracking-widest">
              Detect Absences
            </button>
          </div>
  ...
  <div class="hidden fixed inset-0 bg-black/80 backdrop-blur-md flex items-center justify-center z-50 p-4" id="createPersonnalModal">
    <div class="bg-[#121215] border border-white/10 p-8 rounded-lg w-full max-w-md shadow-2xl max-h-[90vh] overflow-y-auto">
      <div class="flex justify-between items-center mb-6">
        <h2 class="text-white font-black uppercase tracking-widest text-sm">Create New Personnel</h2>
        <button @click="oppenToggle('createPersonnalModal')" class="text-gray-400 hover:text-red-500 transition-colors">
          <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" /></svg>
        </button>
      </div>
      <form @submit.prevent="submitRegistration" class="space-y-4">
        <input type="text" v-model="fullname" placeholder="Full Name" class="w-full bg-[#0f0f12] border border-white/10 p-3 text-white rounded focus:border-[#00babc] outline-none text-sm">
        <input type="text" v-model="campus" placeholder="Campus" class="w-full bg-[#0f0f12] border border-white/10 p-3 text-white rounded focus:border-[#00babc] outline-none text-sm">
        <select v-model="role" class="w-full bg-[#0f0f12] border border-white/10 p-3 text-white rounded focus:border-[#00babc] outline-none text-sm">
          <option value="" disabled selected>Select Role</option>
          <option value="etudiant">Etudiant</option>
          <option value="admin">Admin</option>
          <option value="formateur">Formateur</option>
        </select>
        <input type="url" v-model="link_profile" placeholder="Image URL (Avatar)" class="w-full bg-[#0f0f12] border border-white/10 p-3 text-white rounded focus:border-[#00babc] outline-none text-sm">
        <input type="text" v-model="ville" placeholder="Ville" class="w-full bg-[#0f0f12] border border-white/10 p-3 text-white rounded focus:border-[#00babc] outline-none text-sm">
        <input type="email" v-model="email" placeholder="Email Address" class="w-full bg-[#0f0f12] border border-white/10 p-3 text-white rounded focus:border-[#00babc] outline-none text-sm">
        <input type="password" v-model="password" placeholder="Password" class="w-full bg-[#0f0f12] border border-white/10 p-3 text-white rounded focus:border-[#00babc] outline-none text-sm">
  ...
  <div class="hidden fixed inset-0 bg-black/80 backdrop-blur-md flex items-center justify-center z-50 p-4" id="AssignClassFormatorModal">
    <div class="bg-[#121215] border border-white/10 p-8 rounded-lg w-full max-w-md shadow-2xl">
      <div class="flex justify-between items-center mb-6">
        <h2 class="text-white font-black uppercase tracking-widest text-sm">Assign Formator</h2>
        <button @click="oppenToggle('AssignClassFormatorModal')" class="text-gray-400 hover:text-red-500 transition-colors">
          <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" /></svg>
        </button>
      </div>
  ...
  <div class="hidden fixed inset-0 bg-black/80 backdrop-blur-md flex items-center justify-center z-50 p-4" id="createClassModal">
    <div class="bg-[#121215] border border-white/10 p-8 rounded-lg w-full max-w-md shadow-2xl">
      <div class="flex justify-between items-center mb-6">
        <h2 class="text-white font-black uppercase tracking-widest text-sm">Create New Class</h2>
        <button @click="oppenToggle('createClassModal')" class="text-gray-400 hover:text-red-500 transition-colors">
          <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" /></svg>
        </button>
      </div>
      <form @submit.prevent="submitClassRegistration" class="space-y-4">
        <input type="text" v-model="classename" placeholder="Class Name" class="w-full bg-[#0f0f12] border border-white/10 p-3 text-white rounded focus:border-[#00babc] outline-none text-sm">
        <input type="number" v-model="classecapacity" placeholder="Class Capacity" class="w-full bg-[#0f0f12] border border-white/10 p-3 text-white rounded focus:border-[#00babc] outline-none text-sm">
        <input type="text" v-model="promo" placeholder="Promotion (e.g. 2024/2025)" class="w-full bg-[#0f0f12] border border-white/10 p-3 text-white rounded focus:border-[#00babc] outline-none text-sm">
        <input type="url" v-model="link_logo" placeholder="Logo URL" class="w-full bg-[#0f0f12] border border-white/10 p-3 text-white rounded focus:border-[#00babc] outline-none text-sm">
        <input type="text" v-model="classeCampus" placeholder="Campus Location" class="w-full bg-[#0f0f12] border border-white/10 p-3 text-white rounded focus:border-[#00babc] outline-none text-sm">
        <button type="submit" class="w-full bg-[#00babc] text-[#121215] font-bold py-3 rounded mt-2 hover:bg-[#00d1d3] transition-all uppercase tracking-widest text-xs">
          Create Class
        </button>
      </form>
